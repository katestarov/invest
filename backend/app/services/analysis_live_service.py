...
class AnalysisService:
    ...
    def _build_weighted_scores(self, metrics: dict, macro: dict) -> dict[str, tuple[float, float, str]]:
        ...
        return {
            ...
            "growth": (round(growth, 2), weights["growth"], "Учитываем рост выручки, её динамику и качество FCF."),
            "market": (round(market, 2), weights["market"], "Рыночный импульс учитывает доходность цены за 1 и 5 лет."),
            "macro": (round(macro_score, 2), weights["macro"], "Макросреда корректирует оценку с учётом ставок, инфляции и роста экономики."),
        }
    ...
    def _build_narrative(self, company: str, total_score: float, weighted_scores: dict[str, tuple[float, float, str]]) -> str:
        ...
        return (
            ...
            f"а главным ограничителем сейчас остаётся «{self.scoring_config['labels'][weakest[0]]}»."
        )
